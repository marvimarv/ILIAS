...
class ilPropertyFormGUI extends ilFormGUI
{
    ...
    protected function keepFileUpload(
        ...
        $moved = ilFileUtils::moveUploadedFile($a_tmp_name, $tmp_file_name, $temp_path . "/" . $tmp_file_name);

        /** @var ilFileInputGUI $file_input */
        $file_input = $this->getItemByPostVar($a_field);
        if (!$moved) {
            // Surface the silent failure instead of rendering a confirmation
            // page whose subsequent POST would crash with "Undefined array
            // key" because the temp file is not on disk. See Mantis #47854.
            if ($file_input !== null) {
